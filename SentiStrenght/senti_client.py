...
class sentistrength():
    ...
    def __del__(self):
        if self.sentistrength:
            os.kill(self.sentistrength.pid, signal.CTRL_C_EVENT)
    ...
    def run_server(self, language):
        ...
        try:
            ...
            try:
                ...
                print(os.getcwd())
                command = (
                        "java -jar SentiStrengthCom.jar sentidata ..\\SentiStrenght\\%s\\ listen 8181 scale noDictionary noDeleteExtraDuplicateLetters negatingWordsOccurAfterSentiment" % language)
                self.sentistrength = subprocess.Popen(command, shell=True)
                time.sleep(1)
                sock.connect(('127.0.0.1', self.port))
                self.language = language
            except:
    ...
    def stop_server(self, proc_pid):
        # to run this in console: sentistrength.stop_server(sentistrength, senti.sentistrength.pid)
        process = psutil.Process(proc_pid)
        for proc in process.children(recursive=True):
            proc.kill()
        process.kill()

    def train_senti(self, lang, filepath, iterations):

        self.run_server(lang)
        # command = ("java -jar SentiStrengthCom.jar sentidata SentiStrenght\\%s\ input %s iterations 30" % (
        #     lang, filepath))
        command = (
                "java -jar SentiStrengthCom.jar sentidata ..\\SentiStrenght\\%s\ multi 8 input %s iterations %i noDictionary noDeleteExtraDuplicateLetters explain" % (
            lang, filepath, iterations))
        subprocess.Popen(command, shell=True)
        time.sleep(1)

    def get_sentiment(self, string_to_code):
        url_encoded = urllib.parse.quote(string_to_code)
        request_string = "GET /%s HTTP/1.0 \r\n\r\n" % url_encoded
        sock = self.run_server(self.language)
...
class multisent():
    ...
    def __init__(self, language, startport=8222, cores=-1, batchsize=1000):
        self.language = language
        self.cores = cores
        self.instances = []
    ...
    def start_server(self, port=None, attempts=5):
        ...
        if check_exists(port):
            ...
            return
        command = (
                "java -jar SentiStrengthCom.jar sentidata ..\\SentiStrenght\\%s\\ listen %s scale noDictionary noDeleteExtraDuplicateLetters negatingWordsOccurAfterSentiment" % (
            self.language, port))
        instance = subprocess.Popen(command, shell=True)
        while not check_exists(port):
            time.sleep(1)
            attempts -= 1
            if not attempts:
                logger.warning('failed to start {self.language} server at port {port}'.format(**locals()))
                return False
    ...
    def stop_server(self, port=None, pid=None):
        if port and pid:
            logger.warning("this function requires EITHER a port OR a pid, ignores pid if both")
        if port:
        ...
        if not instance:
            ...
            return False
        instance = instance[0]
        process = psutil.Process(instance['instance'].pid)
        for proc in process.children(recursive=True):
            proc.kill()
        process.kill()
        time.sleep(1)
    ...
    def stop_all(self):
        logger.info("please wait for all processes to close")
    ...
    def run_batch(self, texts):
        ...
        return Parallel(n_jobs=min(self.cores, len(ports)), backend='threading')(
            delayed(query_instance)(port, text) for port, text in self._loop_over(ports, texts))
